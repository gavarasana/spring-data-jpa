...
//    )
    @Query(
            "select tp from TicketPrice tp where tp.basePrice < :maxTicketPrice " +
                    "and tp.ticketType.includesWorkshop = true"
    )
    List<TicketPrice> getTicketPricesThatIncludeWorkshop(@Param("maxTicketPrice") BigDecimal maxTicketPrice);
}
